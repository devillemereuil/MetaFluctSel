...
nchains <- if_else(parallel::detectCores() > 10, 10, 4)
...
# Adding the ZIPoisson
need_zi <- readRDS("../need_zi.rds")
mods <-
    mods %>%
    mutate(tmpID = str_glue("{DataID}_{Species}_{Population}")) %>%
    mutate(Dist = if_else(Dist == "Poisson" & tmpID %in% need_zi,
                          "ZIPoisson",
                          Dist),
           tmpID = NULL)
...
models_year <- c("FLAT", "EXPNULL", "EXPVARINT", "EXPIID", "EXPAR1", "NULL", "VARINT", "IID", "AR1")
stanmods <-
    mods %>%
    select(Dist) %>%
    distinct() %>%
